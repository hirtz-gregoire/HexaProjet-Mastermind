...
public class Partie {
    ...
    public Reponse tourDeJeu(String motPropose) {
        verifieNbEssais();
        nbEssais++;
        Reponse rep = new MotSecret(motADeviner).compareProposition(motPropose);
        if (!isTerminee() && rep.lettresToutesPlacees()){
            done();
        }
        verifieNbEssais();
        return rep;
    ...
    public boolean isTerminee() { return this.partieTerminee; }

    // la partie est terminée
    void done() { this.partieTerminee = true; }
}
